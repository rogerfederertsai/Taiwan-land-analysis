    ...
    if area_col:
        ...
        with c2:
            ...
            if len(all_counts) > 10:
                pie1_data = pd.concat([top_10, pd.Series({'其他': all_counts.iloc[10:].sum()})])
            else:
                pie1_data = all_counts
            
            fig_p1, ax_p1 = plt.subplots(figsize=(10, 8.5))
            ax_p1.pie(pie1_data, labels=pie1_data.index, autopct='%1.1f%%', 
                      colors=plt.get_cmap('tab20')(range(len(pie1_data))), 
                      wedgeprops={'width': 0.5}, startangle=140)
        ...
        if price_col:
            ...
            with c4:
                ...
                ax_p2.pie(price_stats, labels=price_stats.index, autopct='%1.1f%%', 
        ...
        if os.path.exists(geojson_path):
            ...
            if not gdf.empty:
                ...
                folium.TileLayer(
                    ...
                    attr='&copy; 國土測繪圖資服務雲', name='國土測繪電子地圖'
                ).add_to(m)

                folium.Choropleth(
                    geo_data=gdf, data=map_stats, columns=['區名', '筆數'],
